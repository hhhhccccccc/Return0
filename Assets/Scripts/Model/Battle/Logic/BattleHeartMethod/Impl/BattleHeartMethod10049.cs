...
public class BattleHeartMethod10049 : BattleHeartMethodBase
{
    public override void BeforeClash(MomentParamModel paramModel)
    {
        ...
        {
            var skillA = BattleManager.GetUnit(model.AttackID).GetSkill();
            var skillB = BattleManager.GetUnit(model.HitID).GetSkill();
            if (skillA != null && skillB != null)
            {
                ...
                if (count >= 2)
                {
                    BattleBuffManager.AddBuff(Subject, GameConst.Battle.Buff30011, Subject, GetParamInt(0));
                }
            }
        }
    }
