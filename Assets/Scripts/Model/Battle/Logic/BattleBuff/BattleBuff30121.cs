...
public class BattleBuff30121 : BattleBuffBase
{
    protected override void OnBeforeAction()
    {
        ...
        if (costKeyList.Count > 0)
        {
            ...
            if (keyType == BattleKeyType.KeyRight || keyType == BattleKeyType.KeyDown)
            {
                ...
                if (keyType == BattleKeyType.KeyRight && skillType != SkillType.PowerKilling)
                {
                    Subject.AddRandomKey(3, ChangeKeyReason.BuffEffect);
                }

                if (keyType == BattleKeyType.KeyDown && skillType == SkillType.PowerKilling)
                {
                    Subject.AddRandomKey(3, ChangeKeyReason.BuffEffect);
                }
            }
